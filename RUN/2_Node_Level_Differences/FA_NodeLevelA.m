...
m = size(conn_clean(:,:,3,:),4);
n = size(conn_unclean(:,:,3,:),4);
...
%----------Clean-------------
%-- Create Node Strengths Matrix: Each jth column represents a subject,
% each ith row represents the strength of the node.
NSM_CL= zeros(size(conn_clean,1), size(conn_clean,4));
...
(8) = STD of the difference (?)
(9) = number of the node
the same applies to the "Comparison" loops below
%}
...
save (UndergraduateThesis/2_Node_Level_Differences/Output_Local/'FA_Node_Out_A.mat')
